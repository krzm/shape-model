...
public class Vector2SerializationTest
{
    [Theory]
    ...
    [InlineData(-.1d, -1.0d)]
    public void Vector2Serialization(double x, double y)
    {
    ...
    private static IFileTestTemplate<string> SetupTest(double x, double y)
    {
        IFilePath filePath = new FilePath(@"C:\Tests\TestTempFiles", $"Vector2Serialization({x},{y})", "xml");
        IFileTestTemplate<string> test = new SerializationTest(
            filePath
            , new SerializationTestScheme<Vector2>(
                new SerializerXml()
                , new Vector2(x, y)
    ...
    private static IText BuildExpectedXml(double x, double y)
    {
        ...
        var xmlObjBuilder = new XmlObjectBuilder(_xmlObjParts
            ...
            , false).CreateXml();
        var xmlFile = new XmlFile(new IXmlParser[] {
                new XmlFileParser(
